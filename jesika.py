...
if uploaded_file is not None:
    ...
    """, unsafe_allow_html=True)
        ...
        mse = mean_squared_error(y_test, y_pred)
        st.markdown(f"<h5 style='font-weight:bold;'>Mean Squared Error: {mse}</h5>", unsafe_allow_html=True)
        r2_test = r2_score(y_test, y_pred)
        r2_train = r2_score(y_train, lasso.predict(X_train))
        st.markdown(f"<h5 style='font-weight:bold;'>R² Score on Test Set: {r2_test}</h5>", unsafe_allow_html=True)
        st.markdown(f"<h5 style='font-weight:bold;'>R² Score on Training Set: {r2_train}</h5>", unsafe_allow_html=True)
        r2 = r2_score(y_test, y_pred)
        st.markdown(f"<h5 style='font-weight:bold;'>Precision of Data (R² Score): {r2:.2f}</h5>", unsafe_allow_html=True)
